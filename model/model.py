...
class FeatureExtractorLayer(torch.nn.Module):
    ...
    def forward(self, var_learned_f, var_lp_f, con_learned_f, con_lp_f, edge_learned_f, solver_state, edge_rest_lp_f, edge_index_var_con, valid_edge_mask):
...
class FeatureExtractor(torch.nn.Module):
    def __init__(self, num_var_lp_f, out_var_dim, num_con_lp_f, out_con_dim, num_edge_lp_f, out_edge_dim, depth, use_layer_norm = False):
    ...
    def forward(self, var_lp_f, con_lp_f, solver_state, edge_rest_lp_f, edge_index_var_con):
        ...
        for l in range(len(self.layers)):
            var_learned_f, con_learned_f, edge_learned_f = self.layers[l](
                ...
                edge_index_var_con, valid_edge_mask) # TODO: Use residual connections?
        
        return var_learned_f, con_learned_f, edge_learned_f

class PrimalPerturbationBlock(torch.nn.Module):
    def __init__(self, num_var_lp_f, num_con_lp_f, num_edge_lp_f, depth, var_dim, con_dim, edge_dim, use_layer_norm = False, predict_dist_weights = False):
    ...
    def forward(self, solvers, var_lp_f, con_lp_f, 
        ...
        for d in range(len(self.feature_refinement)):
            var_learned_f, con_learned_f, edge_learned_f = self.feature_refinement[d](
                    var_learned_f, var_lp_f, con_learned_f, con_lp_f, edge_learned_f, solver_state, edge_rest_lp_f, edge_index_var_con, valid_edge_mask
                )
            try:
                assert(torch.all(torch.isfinite(var_learned_f)))
                assert(torch.all(torch.isfinite(con_learned_f)))
        ...
        primal_perturbation[var_degree == 0] = 0
...
class DualDistWeightsBlock(torch.nn.Module):
    def __init__(self, num_var_lp_f, num_con_lp_f, num_edge_lp_f, depth, var_dim, con_dim, edge_dim, use_layer_norm = False):
        ...
        self.feature_refinement = torch.nn.ModuleList(self.feature_refinement)
        self.dist_weights_predictor = EdgeUpdater(num_edge_lp_f + edge_dim, 1, 
    ...
    def forward(self, solvers, var_lp_f, con_lp_f, 
        ...
        dist_weights = self.dist_weights_predictor(
            ...
            edge_index_var_con).squeeze() #+ 1e-6
...
class DualFullCoordinateAscent(torch.nn.Module):
    ...
    def forward(self, solvers, var_lp_f, con_lp_f, 
        ...
        net_delta_edge = scatter_sum(new_mm, var_indices)[var_indices] * distribution_weights - new_mm
