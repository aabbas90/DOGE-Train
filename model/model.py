...
class FeatureExtractorLayer(torch.nn.Module):
    ...
    def forward(self, var_learned_f, var_lp_f, con_learned_f, con_lp_f, edge_learned_f, solver_state, edge_rest_lp_f, edge_index_var_con):
        ...
        try:
            assert(torch.all(torch.isfinite(var_lp_f)))
        except:
            breakpoint()
        ...
        try:
            assert(torch.all(torch.isfinite(con_lp_f)))
        except:
            breakpoint()
        ...
        try:
            ...
            breakpoint()
        try:
            assert(torch.all(torch.isfinite(edge_rest_lp_f)))
        except:
            breakpoint()
...
class PrimalPerturbationBlock(torch.nn.Module):
    ...
    def forward(self, solvers, var_lp_f, con_lp_f, solver_state, edge_rest_lp_f, 
                var_learned_f, con_learned_f, edge_learned_f, 
                dist_weights, omega, edge_index_var_con,
                num_dual_iterations, track_grad_after_dual_itr, dual_improvement_slope):
        ...
        dual_cost_perturbation = self.edge_cost_predictor(
            ...
            edge_index_var_con).squeeze() #TODO: Force dual_cost_perturbation sign to agree with mm diff?
        ...
        # Dual iterations
        solver_state = sol_utils.dual_iterations(solvers, solver_state, dist_weights, num_dual_iterations, omega, dual_improvement_slope, track_grad_after_dual_itr)
        ...
        return solver_state, new_mm, var_lp_f
...
class DualFullCoordinateAscent(torch.nn.Module):
    ...
    def forward(self, solvers, var_lp_f, con_lp_f, edge_lp_f, 
        ...
        # Update costs:
        edge_lp_f[:, 0] = edge_lp_f[:, 0] + torch.nn.ReLU()(-net_delta) # lo cost
        edge_lp_f[:, 1] = edge_lp_f[:, 1] - torch.nn.ReLU()(net_delta)  # hi cost
