...
if __name__ == "__main__":
    ...
    #root_dir = '/home/ahabbas/data/learnDBCA/shape_matching/train_split/instances/'
    ...
    for path, subdirs, files in os.walk(root_dir):
        for instance_name in sorted(files):
            ...
            en = time.time()
            per_itr_time = (en - st) / 10.0
            current_stats = {}
            current_stats['time'] = per_itr_time
            current_stats['stat'] = f'file: {rel_path:30}, Per iteration time: {per_itr_time:.3f}, LP obj: {lp_obj:11.3f}, ILP obj: {ilp_obj:.3f}, Percent rel. gap: {rel_gap:.3f}, BDD LB: {bdd_solved_lb:.3f}, Percent BDD LB gap: {rel_gap_bdd:.3f}, Percent BDD Gap Loss: {rel_gap_loss:.3f}, Num cons: {num_cons}, Num BDD layers: {num_layes}'
            print(current_stats['stat'])
            stats.append(current_stats)

    print('\n\n \t\t Sorted stats: ')
    for current_stats in sorted(stats, key=lambda d: d['time']):
        print(current_stats['stat'])
